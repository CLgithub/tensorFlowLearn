...
## 5.1 卷积神经网络简介
* [全连接神经网络图片分类](../3/book2.1.py)
* [简单卷积神经网络图片分类](./book5.1.py)
...
深度学习的一个基本特性就是能够独立地在训练数据中找到有趣的特征，无须人为的特征工程，而这只在拥有大量训练样本时才能实现。<br>
但是如果模型很小， 并做了很好的正则化，同时任务非常简单，那么几百个样本可能就足够了。由于卷积神经网络学到的是局部的、平移不变的特征，它对于感知问题可以高效地利用数据。虽然数据相对较少， 但在非常小的图像数据集上从头开始训练一个卷积神经网络，仍然可以得到不错的结果，而且无须任何自定义的特征工程<br>
...
### 5.2.2 下载数据
可到kaggle下载[猫狗分类数据集](https://www.kaggle.com/c/dogs-vs-cats/data)

### 5.2.3 构建网络
### 5.2.4 数据预处理
[基础网络模型搭建完成](./book5.2.py)
...
👻：思考，只有用深度学习模型处理图像是才有数据**数据增强**技术吗，如果是，为何？

[使用数据增强的模型](./book5.2.5.py)
...
[使用预处理的卷积神经网络：使用VGG16进行特征提取1](./book5_3_1.py)
![](images/5.3-2.png)
* 在卷积基上添加Dense层，并在输入数据上端到端的运行整个模型。有点：可以使用数据增强。缺点：计算量高很多[使用预处理的卷积神经网络：使用VGG16进行特征提取2](./book5_3_2.py)
![](images/5.3-3.png)
...
微调步骤的前3步已经在特征提取2中完成，后两步的model和conv_base可以直接加载或重新训练，[模型微调](./book5_3_3.py)
![](images/5.3-5.png)
...
## 5.4 卷积神经网络的可视化
### 5.4.1 可视化中间激活
对于给定输入，[展示网络中各个卷积层和池化层输出的特征图](./book5_4_1.py)(全部过滤器)
![](images/5.4-1.png)
![](images/5.4-2.png)
![](images/5.4-3.png)
![](images/5.4-4.png)
...
![](images/5.4-5.png)
<center>(左图)试着凭记忆画一辆自行车;(右图)自行车示意图</center>

### 5.4.2 可视化卷积神经网络的过滤器
观察卷积神经网络学到的过滤器，另一种方法是显示每个过滤器所响应的视觉模式。<br>
思路：得到一个输入图像，使得选定过滤器具有最大响应，以便观察<br>
实现方法：通过**在输入空间中进行梯度上升**来实现：从空白输入图像开始，将**梯度下降**应用于卷积神经网络输入图像的值（👻相当于求网络对于输入图像的偏导），其目的是让某个过滤器的响应**最大化**。<br>
具体实现方法：构建一个损失函数，其目的是让某个卷积层的某个过滤器的值最大化；然后，我们要使用随机梯度下降来调节输入图像的值，以便让这个激活值最大化<br>
[显示每个过滤器响应的视觉模式](./book5_4_2.py)
![](images/5.4-6.png)
👻：思考，可以用某层某个过滤器的最大响应图，输入到[5.4.1](./book5_4_1.py)中，观察该过滤器的输出（尝试了一下，无果）

* 这些过滤器可视化包含卷积神经网络的层如何观察世界的很多信息


### 5.4.3 可视化类激活的热力图
**类激活图**（CAM，class activation map）可视化有助于：

* 了解一张图像的哪一部分让神经网络做出了最终决策
* 对卷积神经网络的决策过程进行调试
* 定位图像中的特定目标

类激活热力图是与**特定输出类别**相关的**二维分数网格**，它表示每个位置对该类别的重要程度

实现方法：给定一张输入图像，对于一个卷积层的输出特征图，用 **类别** 相对于 **通道** 的 **剃度**，用这个剃度对这个特征图中的每个通道进行加权（👻：相当于看哪个通道更与类别具有相关性，就对哪个通道进行空间加权，就能得到“输入图像对类别的激活强度”的空间图）
[可视化类激活的热力图](./book5_4_3.py)
![](images/5.4-7.jpg)
...
[将此方法应用于自己的模型](./book5_4_3-2.py)<br>
![](images/5.4-8.jpg)
