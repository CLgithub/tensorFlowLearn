...
#构建网络
def getModel(larer1Num,larer2Num):
    model=models.Sequential()   #创建模型
    model.add(layers.Dense(larer1Num, activation='relu', input_shape=(10000,)))    #第一层，输入(s,10000),输出(s,16)
    model.add(layers.Dense(larer2Num, activation='relu' ))
...
history1=getModel(16,16)
loss1,acc1=getlossAndAcc(history1)
history2=getModel(4,4)
loss2,acc2=getlossAndAcc(history2)
history3=getModel(512,512)
loss3,acc3=getlossAndAcc(history3)

epochs=range(1, len(loss1)+1)
def show1(loss1, loss2, acc1, acc2):
    plt.plot(epochs, loss1, 'b', label='loss_Original')
    plt.plot(epochs, loss2, 'c', label='loss_Smaller')
    plt.xlabel('Epochs')
    plt.ylabel('loss')
    plt.legend()
    plt.show()
    ...
    plt.clf()   #清空图像
    plt.plot(epochs, acc1, 'b', label='acc_Original')
    plt.plot(epochs, acc2, 'c', label='acc_Smaller')
    plt.xlabel('Epochs')
    plt.ylabel('acc')
    plt.legend()
    plt.show()

show1(loss1, loss3, acc1, acc3)
